...
public class NestedChildLinearLayout extends LinearLayout {
    private static final String TAG = "Child";

    public NestedChildLinearLayout(Context context) {
        super(context);
        init();
    }

    public NestedChildLinearLayout(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public NestedChildLinearLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    private float mDownX;//手指第一次落下时的x位置
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch ( ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mDownX = x;
                mDownY = y;
                startNestedScroll(ViewCompat.SCROLL_AXIS_NONE);
                break;

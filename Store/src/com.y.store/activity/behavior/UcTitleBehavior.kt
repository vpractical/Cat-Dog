...
class UcTitleBehavior (context: Context, attrs: AttributeSet): CoordinatorLayout.Behavior<LinearLayout>(context,attrs) {
    ...
    override fun onDependentViewChanged(parent: CoordinatorLayout, child: LinearLayout, dependency: View): Boolean {
        ...
        val 比例 = titleHeight / (dependency.measuredHeight - titleHeight - tabHeight)

        var transY = dependency.translationY * 比例

        if(transY > titleHeight){
            transY = titleHeight
        }
